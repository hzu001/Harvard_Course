...
library(tidyverse)
url <- url <- "https://archive.ics.uci.edu/ml/machine-learning-databases/breast-cancer-wisconsin/wdbc.data"
dat <- read_csv(col_names=FALSE,url)
nrow(dat)
ncol(dat)
...
head(AwardsPlayers)
blub <- AwardsPlayers %>% filter(yearID == 2016)
head(top_names)
head(blub)

left_join(top_names, blub, by="playerID")

length(setdiff(blub$playerID, top_names$playerID))
...
tab <- html_table(nodes[[19]]) %>% setNames(c("Team", "Payroll", "Average"))
...
blub <- full_join(tab_1,tab_2, by="Team")
nrow(blub)
...
sapply(tab[1:10], html_table,fill=TRUE)
...
s <- '10"'    # correct to write 10 followed by double quotes
...
#### Section 3.2: String Processing Part2 #####
...
brexit_polls %>% filter(grepl("\\d+-04-\\d+", startdate))
#alternatively: sum(month(brexit_polls$startdate) == 4)

brexit_polls %>% mutate(enddate = round_date(enddate, unit = "week")) %>% filter(enddate=="2016-06-12")
#alternatively: sum(month(brexit_polls$startdate) == 4)

brexit_polls %>% mutate(enddate = weekdays(enddate)) %>% count(enddate)
#alternatively: table(weekdays(brexit_polls$enddate))

data(movielens)

movielens %>% mutate(timestamp = as_datetime(timestamp), year=year(timestamp)) %>% group_by(year) %>% summarise(total_reviews = n())
#alternatively: dates <- as_datetime(movielens$timestamp)
#reviews_by_year <- table(year(dates))  
#names(which.max(reviews_by_year)) 


movielens %>% mutate(timestamp = as_datetime(timestamp), hour=hour(timestamp)) %>% group_by(hour) %>% summarise(total_reviews = n()) %>% filter(total_reviews == max(total_reviews))
#alternatively: reviews_by_hour <- table(hour(dates))    # count reviews by hour
...
words <- words %>% filter(!word %in% stop_words$word & !str_detect(word, "^\\d$"))

words %>% count(word,sort=TRUE) %>% filter(n>100) %>% nrow()
...
# Count the number of words with positive values
positive_words <- afinn_sentiments %>%
  filter(value > 0) %>%
  nrow()
total_words <- nrow(afinn_sentiments)
3560/6353
